...
window.addEventListener("scroll", () => {
  ...
  // We also need to grab the window scrollY position since we will be moving the elements based on this Number.
  ...
  let showcaseDataStyles = `
        transform: translateY(${scrollPositionY * 0.2}%);
  ...
  if (scrollPositionY < 600) {
    ...
    imageTop.style.transform = `translateY(${-scrollPositionY * 0.001}%)`;
    imageMiddle.style.transform = `translateY(${scrollPositionY * 0.02}%)`;
    imageBottom.style.transform = `translateY(${-scrollPositionY * 0.04}%)`;
  }
});
